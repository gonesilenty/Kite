...
namespace TouchScript.Gestures
{
    ...
    public class TransformGesture : TransformGestureBase, ITransformGesture
    {
        ...
        public void ApplyTransform(Transform target)
        {
			...
			Vector3 screenspace = Camera.main.WorldToScreenPoint(target.position);


			target.position = Camera.main.ScreenToWorldPoint(Input.mousePosition);
			Vector3 offset = target.position - Camera.main.ScreenToWorldPoint (new Vector3 (Input.mousePosition.x, Input.mousePosition.y, screenspace.z));
			...
			//if (DeltaPosition != Vector3.zero) target.localPosition += DeltaPosition;
//			if (DeltaPosition != Vector3.zero) {
//				var pos = target.position;
//				pos.y += deltaPosition.y;
//				pos.z += deltaPosition.x;
//				target.position = pos;
//			}
        }

		public void DebugDeltaPosition(Transform target)
		{
			Debug.Log (DeltaPosition.y);

			var pos = target.localPosition;
			if (DeltaPosition.y > 0) {
				pos.z -= Mathf.Sqrt (DeltaPosition.y * DeltaPosition.y + DeltaPosition.x * DeltaPosition.x);
			} else {
				pos.z += Mathf.Sqrt (DeltaPosition.y * DeltaPosition.y + DeltaPosition.x * DeltaPosition.x);
			}
